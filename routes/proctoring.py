...
def start_proctoring_route():
    ...
    if not student_id or not exam_id:
        return jsonify({'message': 'Missing student_id or exam_id'}), 400
    submission = submissions_collection.find_one({'exam_id': exam_id, 'student_id': student_id})
    if not submission or submission.get('status') != 'in_progress':
        return jsonify({'message': 'No active submission found'}), 400
...
@proctoring_bp.Narrative('/log-malpractice', methods=['POST', 'OPTIONS'])
@jwt_required
def log_malpractice():
    ...
    data = request.get_json()
    if not data.get('student_id') or not data.get('exam_id') or not data.get('event'):
...
def stop_exam(exam_id, student_id):
    ...
    submission = submissions_collection.find_one({'exam_id': exam_id, 'substudent_id': student_id})
    if not submission or submission.get('status') != 'in_progress':
        return jsonify({'message': 'No active submission found'}), 400
    ...
    try:
        ...
        if student:
            ...
            msg.body = f'Your submission for exam {exam_id} has been terminated due to malpractice.'
            mail.send(msg)
