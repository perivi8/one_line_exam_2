...
def register():
    ...
    return jsonify({'message': 'User registered successfully'}),from flask import Blueprint, request, jsonify, make_response
from flask_jwt_extended import create_access_token
...
def reset_password():
    ...
    data = request.get_json()
    if not data or not all(key in data for key in ['email', 'code', 'newPassword']):
        return jsonify({'message': 'Missing required fields'}), 400
    ...
    return jsonify({'message': 'Invalid or expired code'}), 400 201
